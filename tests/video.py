...
def suite(browser_type):
    ...
    check(browser_type.name+' title video',page.locator('#openingVideo').count()==1);page.wait_for_function('openingVideo.readyState>=2 && openingVideo.videoWidth>0',timeout=10000);check(browser_type.name+' title loaded',page.evaluate("openingVideo.readyState>=2 && openingVideo.muted && openingVideo.playsInline"))
    ...
    page.wait_for_function("repairMovie.readyState>=2 && repairMovie.videoWidth>0 && repairMovie.currentSrc.includes('repair-1.mp4')",timeout=10000)
    ...
    page.click('#viewerMovie');page.wait_for_selector('#movieViewer.on')
    page.wait_for_function("repairMovie.readyState>=2 && repairMovie.videoWidth>0 && repairMovie.currentSrc.includes('repair-1.mp4')",timeout=10000)
    check(browser_type.name+' replay',page.evaluate("repairMovie.currentSrc.includes('repair-1.mp4')"))
