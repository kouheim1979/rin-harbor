...
def test_origin_offline(browser, root, check, reload_page):
    ...
    try:
        ...
        page.evaluate("S.auto=false;S.autoStory=false;S.coins=2345;S.repair=5;S.warehouse={fish6:2};saveNow()")
        keys = page.evaluate("async()=>{const c=await caches.open('rin-harbor-20260908-atelier4');return (await c.keys()).map(r=>r.url)}")
        ...
        check(label+'warehouse restored offline',page.evaluate('S.warehouse.fish6===2'))
        ...
        page.evaluate("closeAlbum();setView('warehouse');takeFromWarehouse('fish6');S.coins=3456;saveNow()")
        reload_page(page)
        check(label+'second offline reload saves progress', page.evaluate('S.coins===3456&&S.repair===5'))
        check(label+'warehouse withdrawal persists offline',page.evaluate('S.warehouse.fish6===1&&countBoard().fish6===1'))
