...
def asset_checks():
    ...
    check('Live CSS equals tested CSS',hashlib.sha256(fetch('youth.css?v=video1')).digest()==hashlib.sha256((ROOT/'youth.css').read_bytes()).digest())
    check('Live vector art equals tested vector art',hashlib.sha256(fetch('item-art.js?v=video1')).digest()==hashlib.sha256((ROOT/'item-art.js').read_bytes()).digest())
...
def suite(browser,engine):
    ...
    try:
        ...
        page.wait_for_function("typeof RELEASE!=='undefined' && RELEASE==='20260908-video1'")
...
if __name__=='__main__':
    try:
        ...
        raise
    finally:(OUT/'report.json').write_text(json.dumps({'base_url':URL,'release':'20260908-video1','checks':RESULTS},ensure_ascii=False,indent=2))
