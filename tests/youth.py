...
def asset_checks():
    ...
    check('Live CSS equals tested CSS',hashlib.sha256(fetch('youth.css?v=guide1')).digest()==hashlib.sha256((ROOT/'youth.css').read_bytes()).digest())
    check('Live vector art equals tested vector art',hashlib.sha256(fetch('item-art.js?v=guide1')).digest()==hashlib.sha256((ROOT/'item-art.js').read_bytes()).digest())
...
if __name__=='__main__':
    try:
        ...
        raise
    finally:(OUT/'report.json').write_text(json.dumps({'base_url':URL,'release':'20260908-guide1','checks':RESULTS},ensure_ascii=False,indent=2))
